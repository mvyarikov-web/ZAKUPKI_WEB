...
# Инструкция для создания постановок с помощью Specify:

Если в начале запроса стоит слово «Постановка», необходимо действовать по следующему сценарию:

Given: пользователь формирует постановку через Specify.
When: создаётся новый increment-файл.
Then: он появляется в папке increments_doc с корректным именем по шаблону NNN_increment_YYYY-MM-DD.md, заполненными разделами «Постановка» и «Спецификация».
And: после первого коммита в файле автоматически проставляются дата/время (MSK) и commit hash; раздел «Реализация» содержит краткое описание изменений.

# Инструкции для GitHub Copilot — проект ZAKUPKI_WEB

Этот репозиторий — веб-интерфейс (Flask) для автоматизированного поиска по документам. Внутри — модуль `document_processor` для индексации и поиска по консолидационному файлу `_search_index.txt` с поддержкой вложенных папок, ZIP/RAR и базового OCR.
...
## Общие принципы
- Вся проектная документация по постановкам и спецификациям ведётся в формате *журнала приращений* (increments).
- Источником истины является директория `increments_doc/`, содержащая последовательные increment-файлы.
- Формат имени: `NNN_increment_YYYY-MM-DD.md`, где `NNN` — номер приращения (000, 001, …), а дата — дата создания или фиксации инкремента.
- Все ранее созданные increment-файлы должны быть переименованы по этому шаблону.
- Вся документация ведётся **на русском языке**, в кодировке UTF-8.
...
- Приложение по умолчанию слушает на порту 8081 (dev). Перед перезапуском сервера освобождай порт, если он занят.
- Команды (macOS, zsh):
