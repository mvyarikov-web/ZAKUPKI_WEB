...
- Формат имени: `YYYY-MM-DD_HH-MM_increment-NNN.md`, где `NNN` — номер приращения (000, 001, …).
- Вся документация ведётся **на русском языке**, в кодировке UTF-8.

### Нулевое приращение
- Файл `increment-000.md` создаётся для консолидации всей ранее существующей документации (из папок `specs/`, `docs/` и аналогичных).
- После консолидации исходные папки и материалы удаляются, кроме:
	- `constitution.md`
	- `copilot-instructions.md`
	- `README.md`.

### Новые приращения
- Каждое новое приращение начинается с постановки, сформированной через **Specify**.
- На основе постановки создаётся новый increment-файл по шаблону, в который автоматически вносятся:
	- **Постановка** и **Спецификация** (из Specify).
	- После первого коммита: **дата и время пуша (MSK)** и **commit hash**.
	- **Реализация (простыми словами)** добавляется автоматически или вручную.

### Шаблон increment-файла
```markdown
...
(что сделано и зачем, коротко и понятно)  
