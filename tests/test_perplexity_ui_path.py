...
def test_ui_path_with_search_enabled():
    ...
    for phrase in forbidden_phrases:
        if phrase in answer_lower:
            ...
            print("  1. '🔍 DEBUG: search_params до нормализации'")
            print("  2. '🔍 DEBUG: search_requested = True'")
            print("  3. '🌐 Режим С ПОИСКОМ: extra_body = ...'")
            return False
    ...
    if search_results:
        ...
        print("Это может означать, что поиск не был выполнен.")
    ...
    if found_indicators:
        ...
        print("  ⚠️  Не найдены явные признаки поиска")
    
    # Итоговая проверка
    if search_results or found_indicators:
        print("\n✅ УСПЕХ: Поиск был выполнен!")
        print("Модель ответила корректно, не отказываясь от поиска.")
        return True
    else:
        print("\n⚠️  СОМНИТЕЛЬНЫЙ РЕЗУЛЬТАТ:")
        print("Модель не отказалась, но нет явных признаков поиска.")
        print("Проверьте логи для подтверждения.")
