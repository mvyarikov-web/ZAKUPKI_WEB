# increment-014 — Параллелизация OCR и UI прогресс-бара

Дата и время пуша (MSK): 2025-10-19 13:17:25
Git commit hash: efbeec734b65c9fa1d1280e1db81ba9ed2b49429

## Реализация (простыми словами)

Реализована групповая индексация файлов с возможностью поиска по мере обработки:

1. **Классификация файлов**: Файлы автоматически разделяются на 3 группы:
   - FAST (TXT, CSV, HTML, JSON, XML) — обрабатываются мгновенно
   - MEDIUM (DOCX, XLSX, XLS, DOC, векторные PDF) — обрабатываются быстро
   - SLOW (PDF со сканами, архивы ZIP/RAR) — требуют OCR и времени

2. **Резервация секций**: При запуске индексации создаётся индекс с заголовками всех групп и маркерами `<!-- BEGIN_GROUP -->` / `<!-- END_GROUP -->`.

3. **Поэтапная обработка**: Каждая группа обрабатывается отдельно во временный файл, затем атомарно вставляется в главный индекс.
...
7. **Обратная совместимость**: Классическая индексация (без групп) продолжает работать при `use_groups=false`.
...
- `webapp/routes/search.py`: обновлены эндпоинты `/index_status` и `/view_index`, добавлена функция `_parse_index_groups()`
...
**НОВАЯ АРХИТЕКТУРА (increment-014):**
...
#### Шаг 2.4: Обновление очистки индекса
...
2. **JavaScript опрос** (`static/js/script.js`):
   ```javascript
...
3. **HTML элементы** (`templates/index.html`):
   ```html
...
- Индикаторы групп обновляются: ⏸ → ⏳ → ✅
- Подсказка показывает, когда поиск доступен
- После завершения FAST группы пользователь может выполнить поиск

#### Шаг 2.6: Интеграция с кнопкой "Просмотр индекса"

**Обновление кнопки в `templates/index.html`:**
```html
<!-- Старая кнопка -->
...
<script>
// Обновляем подсказку о размере индекса
async function updateIndexSizeHint() {
    try {
        ...
        if (data.index_exists && data.groups_info) {
            ...
            hint.textContent = `${totalFiles} файлов, ${completedGroups}/3 групп`;
            hint.style.color = completedGroups === 3 ? '#28a745' : '#ffc107';
        } else {
            hint.textContent = 'Индекс не создан';
            hint.style.color = '#6c757d';
        }
    } catch (e) {
        console.error('Ошибка обновления подсказки:', e);
    }
}

function viewIndexWithAutoRefresh() {
    // Открываем в новом окне с автообновлением
    const win = window.open('/view_index_page', '_blank', 'width=1200,height=800');
    
    // Если индексация идёт, подсвечиваем кнопку
...
// Обновляем подсказку каждые 3 секунды
setInterval(updateIndexSizeHint, 3000);
updateIndexSizeHint();
</script>
```

**Новый роут для страницы просмотра:**
```python
@search_bp.get('/view_index_page')
def view_index_page():
    """Страница просмотра индекса с автообновлением (из Шага 2.3)."""
...
### 🗂️ Задача 3: Иерархическая структура индекса и динамическое обновление (ПРИОРИТЕТ)

**Цель:** Индекс должен отражать структуру папок/архивов как в UI, обновляться при удалении файлов/папок, и подсвечивать найденные слова при просмотре.

#### Шаг 3.1: Иерархическая структура в индексе

**Новая структура индекса (фрагмент):**

```
════════════════════════════════════════
[ГРУППА: FAST] TXT, CSV, HTML
Файлов: 50 | Статус: ✅ завершено
════════════════════════════════════════
<!-- BEGIN_FAST -->

📁 Папка: Договоры/2024
────────────────────────────────────────
ЗАГОЛОВОК: Договоры/2024/договор_001.txt
Формат: TXT
Источник: uploads/Договоры/2024/договор_001.txt
────────────────────────────────────────
Текст договора №001 от 15 января 2024 года...
────────────────────────────────────────

ЗАГОЛОВОК: Договоры/2024/договор_002.txt
Формат: TXT
Источник: uploads/Договоры/2024/договор_002.txt
────────────────────────────────────────
Текст договора №002 от 20 января 2024 года...
────────────────────────────────────────

📁 Папка: Отчеты
────────────────────────────────────────
ЗАГОЛОВОК: Отчеты/отчет_январь.csv
Формат: CSV
Источник: uploads/Отчеты/отчет_январь.csv
────────────────────────────────────────
Дата,Сумма,Контрагент
2024-01-15,100000,ООО "Компания"...
────────────────────────────────────────

📦 Архив: Документы.zip
────────────────────────────────────────
ЗАГОЛОВОК: Документы.zip/readme.txt
Формат: TXT (из архива)
Источник: uploads/Документы.zip/readme.txt
────────────────────────────────────────
Содержимое readme файла из архива...
────────────────────────────────────────
...
- 📁 Заголовки папок группируют файлы визуально.
- 📦 Заголовки архивов выделяют файлы из ZIP/RAR.
- 🔗 Относительные пути совпадают с UI (webkitdirectory/архив как виртуальная папка).
- 🧭 Порядок в индексе соответствует порядку в дереве UI.

Реализация: в `_process_group()` сортировать по `rel_path` и печатать заголовок папки/архива при смене префикса.

#### Шаг 3.2: Метод записи с учётом иерархии

Обновление `_write_entry(...)` (идея):
```python
def _write_entry(self, out: TextIO, rel_path: str, text: str, meta: Dict,
    ...
    if folder and folder != current_folder:
        icon = '📦' if meta.get('from_archive') else '📁'
        out.write(f"\n{icon} Папка: {folder}\n")
        out.write('─' * 80 + '\n')
    out.write(f"ЗАГОЛОВОК: {rel_path}\n")
    out.write(f"Формат: {meta.get('format','неизвестно')}\n")
    source = meta.get('source', f"uploads/{rel_path}")
    out.write(f"Источник: {source}\n")
    out.write('─' * 80 + '\n')
...
#### Шаг 3.3: Динамическое обновление индекса при удалении

При удалении файла/папки индекс должен обновляться без полной пересборки.

Идея функции `_update_index_after_deletion(deleted_paths: List[str]) -> bool`:
1) Прочитать `uploads/_search_index.txt`.
2) Для каждого пути удалить блок от `ЗАГОЛОВОК: <rel>` до следующего `ЗАГОЛОВОК:`/`📁 Папка:`/`📦 Архив:`/`<!-- END_`.
3) Если удалена папка — удалить заголовок `📁 Папка: <folder>` если внутри не осталось файлов.
4) Записать файл атомарно (`.tmp` → `os.replace`).

Вызовы:
- После `DELETE /delete/<path>` — с одним путём.
- После `DELETE /delete_folder/<path>` — со списком всех файлов внутри + сам заголовок папки.

Если индекс отсутствует или формат неожиданен — вернуть False и предложить `/build_index`.

#### Шаг 3.4: Подсветка слов при просмотре индекса

`GET /view_index` принимает `highlight=слово1,слово2`.
Фильтрация служебных строк остаётся как для `raw=0`.
Подсветка применяется к тексту записей (не к заголовкам) — обрамление матчей специальными символами или HTML-классом при рендере страницы просмотра (`/view_index_page`).

Подсказка для UI:
- При выполнении поиска сохранять термины в `localStorage` и передавать их в `view_index` (как query-параметр) для подсветки.
...
**Цель:** Ускорить OCR на многостраничных сканах (3+ страницы) без деградации стабильности.
