...
**Чек-лист для Copilot**
- [x] Реализовать `scripts/migrate_files_to_db.py` с переносом `uploads/`, `index/api_keys.json`, `index/models.json`.
- [x] Добавить `MIGRATION_GUIDE.md` и обновить `README.md` (раздел о запуске БД, dual-mode, rollback).
- [x] Собрать `pytest -q` и smoke-тест `/health` (базовая проверка готова).
