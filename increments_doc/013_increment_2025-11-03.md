...
- [ ] Обновить `requirements.txt`, добавив `sqlalchemy`, `alembic`, `cryptography`, `bcrypt`, `PyJWT`, `python-dotenv` с зафиксированными версиями.
- [ ] Установить зависимости в `.venv` и убедиться, что `pip check` проходит без ошибок.
- [ ] Добавить быстрый импортный тест (`tests/test_dependencies_import.py`).

**Чек-лист для ручной проверки**
- [ ] Выполнить `pip freeze | grep -E "(SQLAlchemy|alembic|cryptography|bcrypt|PyJWT|python-dotenv)"` и убедиться, что версии совпадают со спецификацией.
- [ ] Запустить `python -m pip check` и зафиксировать отсутствие конфликтов.
- [ ] Убедиться, что `pytest tests/test_dependencies_import.py -q` завершается успешно.

### Шаг 2. Настраиваем переменные окружения

**Чек-лист для Copilot**
- [ ] Создать `.env.sample` с обязательными ключами (`DATABASE_URL`, `FERNET_ENCRYPTION_KEY`, `JWT_SECRET_KEY`).
- [ ] Реализовать загрузку настроек через `python-dotenv` и `ConfigService`.
- [ ] Добавить fail-fast проверки при отсутствии ключевых переменных.
- [ ] Написать тест `tests/test_config_env.py` на сценарии с отсутствием переменных.

**Чек-лист для ручной проверки**
- [ ] Скопировать `.env.sample` в `.env` и заполнить значения.
- [ ] Запустить `pytest tests/test_config_env.py -q`.
- [ ] Запустить приложение без `.env` и убедиться, что оно корректно завершается с понятной ошибкой.

### Шаг 3. Инициализируем Alembic

**Чек-лист для Copilot**
- [ ] Выполнить `alembic init alembic` (через автоматизацию) и добавить каталоги в репозиторий.
- [ ] Сконфигурировать `alembic.ini` на чтение `DATABASE_URL` из настроек приложения.
- [ ] Обновить `alembic/env.py` с использованием `SQLAlchemy` `MetaData` из проекта.

**Чек-лист для ручной проверки**
- [ ] Запустить `alembic upgrade head` на чистой БД — команда не должна падать (пока таблиц нет).
- [ ] Проверить структуру каталогов `alembic/` и наличие `versions/`.
- [ ] Просмотреть `alembic.ini` на отсутствие хардкода URL.

### Шаг 4. Собираем базовый слой SQLAlchemy

**Чек-лист для Copilot**
- [ ] Создать `webapp/db/base.py` с `DeclarativeBase`, `engine`, `sessionmaker` и фабрикой сессий.
- [ ] Добавить `ScopedSession` для использования в Flask (middleware).
- [ ] Написать модульный тест `tests/test_db_session.py` с проверкой открытия/закрытия сессий.

**Чек-лист для ручной проверки**
- [ ] Запустить `pytest tests/test_db_session.py -q`.
- [ ] Проверить, что `Base.metadata` подтягивается в Alembic.
- [ ] Убедиться, что при запросе к БД соединение закрывается после выхода из контекста.

### Шаг 5. Описываем модели данных

**Чек-лист для Copilot**
- [ ] Заполнить `webapp/db/models.py` сущностями из раздела 1.1 (users, sessions, documents, chunks, api_keys, user_models, search_history, app_logs, job_queue).
- [ ] Добавить `__tablename__`, индексы и связи.
- [ ] Создать тесты `tests/test_db_models_schema.py` для проверки наличия ключевых полей и ограничений.

**Чек-лист для ручной проверки**
- [ ] Запустить `pytest tests/test_db_models_schema.py -q`.
- [ ] Проверить, что модели не вызывают кольцевых импортов.
- [ ] Просмотреть docstring'и на соответствие спецификации.
...
**Чек-лист для Copilot**
- [ ] Создать каталог `webapp/db/repositories/` с `users.py`, `documents.py`, `ai.py`, `search.py`, `logs.py`, `jobs.py`.
- [ ] Добавить базовый класс `BaseRepository` для работы с сессиями.
- [ ] Написать unit-тесты `tests/repositories/test_users_repo.py` и `test_documents_repo.py` с использованием фикстуры временной БД.

**Чек-лист для ручной проверки**
- [ ] Запустить точечные тесты репозиториев.
- [ ] Проверить, что репозитории не обращаются напрямую к Flask `request`.
- [ ] Вручную просмотреть CRUD-операции на соответствие требованиям из раздела 1.2.

### Шаг 8. Включаем dual-mode адаптер

**Чек-лист для Copilot**
- [ ] Реализовать флаг `use_database` в конфиге.
- [ ] Добавить адаптер, умеющий брать данные из БД и синхронизировать `_search_index.txt` для legacy UI.
- [ ] Написать тест `tests/test_dual_mode_adapter.py`, сравнивающий результаты поиска в обоих режимах.

**Чек-лист для ручной проверки**
- [ ] Запустить `pytest tests/test_dual_mode_adapter.py -q`.
- [ ] Проверить, что приложение стартует с `use_database=false` и `true`.
- [ ] Убедиться, что индекс пересоздаётся при переключении режима.

### Шаг 9. Реализуем AuthService и JWT-поток

**Чек-лист для Copilot**
- [ ] Создать `AuthService` с регистрацией, входом, валидацией пароля (bcrypt) и выдачей JWT.
- [ ] Добавить middleware, записывающее `request.user`.
- [ ] Написать интеграционный тест `tests/test_auth_flow.py`.

**Чек-лист для ручной проверки**
- [ ] Выполнить `pytest tests/test_auth_flow.py -q`.
...
**Чек-лист для Copilot**
- [ ] Создать репозиторий `ApiKeysRepository` и сервис для шифрования Fernet.
- [ ] Переписать `utils/api_keys_manager_multiple.py` на работу через БД.
- [ ] Добавить тест `tests/test_api_keys_storage.py` с проверкой шифрования/расшифровки.

**Чек-лист для ручной проверки**
- [ ] Запустить `pytest tests/test_api_keys_storage.py -q`.
...
**Чек-лист для Copilot**
- [ ] Переписать маршруты `/upload` и связанные сервисы на работу с `documents` (bytea + sha256).
- [ ] Добавить проверку размеров, MIME и дедупликации согласно разделу 2.1.
- [ ] Написать интеграционный тест `tests/test_documents_upload_db.py`.

**Чек-лист для ручной проверки**
- [ ] Выполнить `pytest tests/test_documents_upload_db.py -q`.
...
**Чек-лист для Copilot**
- [ ] Перенести воркер индексации на чтение из БД и запись чанков/эмбеддингов в `chunks`.
- [ ] Подключить pgvector и реализовать сохранение `embedding`.
- [ ] Написать тест `tests/test_indexing_pgvector.py` (без OCR).

**Чек-лист для ручной проверки**
- [ ] Выполнить `pytest tests/test_indexing_pgvector.py -q`.
...
**Чек-лист для Copilot**
- [ ] Реализовать `SearchService`, читающий из `chunks` и записывающий результаты в `search_history`.
- [ ] Добавить гибридный поиск (keyword + semantic) с параметрами.
- [ ] Написать тест `tests/test_search_service_db.py`.

**Чек-лист для ручной проверки**
- [ ] Выполнить `pytest tests/test_search_service_db.py -q`.
...
**Чек-лист для Copilot**
- [ ] Внедрить `DatabaseLogHandler`, пишущий события в `app_logs`.
- [ ] Добавить middleware для аудита HTTP-запросов.
- [ ] Написать тест `tests/test_logging_db.py`.

**Чек-лист для ручной проверки**
- [ ] Выполнить `pytest tests/test_logging_db.py -q`.
- [ ] Создать искусственно ошибку и убедиться, что она попадает в `app_logs`.
- [ ] Проверить наличие маскирования секретов в логах.
