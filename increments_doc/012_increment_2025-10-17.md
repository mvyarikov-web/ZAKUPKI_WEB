...
Дата и время первого пуша (MSK): 2025-10-18 17:15:46 MSK
Git commit hash (первый): dd075d534cc43b67d0c73c4b8faab89ba122597a

Дата и время второго пуша (MSK): 2025-10-18 19:32:15 MSK
Git commit hash (второй): 70d590cff0c11490a4e73f8b1040766cd00ee414
...
4. **Обработка PDF из архивов (ZIP/RAR) — КРИТИЧНО:**
...
### Критерии готовности:
- ✅ Создана отдельный модуль pdf_reader с чистым API
...
- ✅ Обновлён pytest.ini (добавлены новые тесты в python_files, включая test_pdf_from_archive.py)
...
## Второй коммит (18.10.2025): Исправление OCR и улучшение автокоррекции ориентации

**Проблема:**
При попытке распознавания сканированных PDF-документов возникала ошибка:
```
PDFInfoNotInstalledError: Unable to get page count. Is poppler installed and in PATH?
```

Библиотека `pdf2image` требует системную утилиту **poppler** для конвертации PDF в изображения. Без неё OCR не работает.

**Дополнительная проблема:**
Метод `_auto_orient_image()` выбирал неправильную ориентацию документа:
- Tesseract OSD рекомендовал поворот на 270°, но это давало перевёрнутый текст
- Эвристика считала "мусорные" символы как "читаемые"

**Что исправлено:**

1. **Установлен Poppler:**
   - macOS: `brew install poppler`
   - Linux: `apt-get install poppler-utils` или `dnf install poppler-utils`
   - Обновлена документация `docs/TESSERACT_INSTALL.md`

2. **Улучшен алгоритм автокоррекции ориентации:**
   - Отказ от ненадёжного Tesseract OSD
   - Новая эвристика оценки качества:
     * Подсчёт кириллических/латинских символов (не всех alnum)
     * Штраф за "мусорные" символы
     * Штраф за отсутствие букв
   - Формула: `score = total_chars * useful_ratio * letter_ratio`

3. **Результаты тестирования** (документ "ТЗ поставка сплит-систем.pdf"):
...
4. **Добавлена документация:**
   - `docs/OCR_POPPLER_FIX.md` — описание проблемы и решения
   - `docs/OCR_AUTO_ORIENT_FIX.md` — детали улучшения автокоррекции
   - Обновлён `docs/TESSERACT_INSTALL.md` с инструкциями по установке Poppler

5. **Добавлен тест:**
   - `tests/test_ocr_auto_orient.py` — проверка автокоррекции ориентации
...
- **poppler** — конвертация PDF в изображения ⭐ НОВОЕ
...
reader = PdfReader()
result = reader.read_pdf('scan.pdf', ocr='auto', lang='rus+eng')
print(f"OCR использовался: {result['ocr_used']}")
print(f"Извлечено: {len(result['text'])} символов")
```

Сервер запущен на http://127.0.0.1:8081
- Новых тестов: +11
- Сломанных тестов: 0
