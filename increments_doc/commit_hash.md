...
feat: утилита полной очистки БД (Clean_DB.py) и безопасный скрипт reset_rag_db.py
Clean_DB.py: удобный лаунчер для вызова очистки с подтверждением, чтением .env и venv
scripts/reset_rag_db.py: безопасный TRUNCATE бизнес-таблиц с RESTART IDENTITY CASCADE, вывод статистики до/после
Не затрагивает пользователей и сессии, только документы, чанки, историю, логи, промпты, ключи, модели