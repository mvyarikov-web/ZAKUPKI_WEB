...
## Веб-слой (Flask)
- Роуты:
	- POST `/upload` — сохраняй в `uploads/` с безопасными именами; поддерживай вложенные папки через webkitdirectory (если есть).
	- POST `/build_index` — явно пересобирай индекс для `uploads/`.
	- POST `/search` — используй индекс; при отсутствии — создай автоматически.
	...
	- GET `/index_status` — статус наличия и размера `uploads/_search_index.txt`.
